...
This template includes configuration for Python and TypeScript. After creating a project from this template:

- **Remove unused config files**: Delete `pyproject.toml` if not using Python, `package.json`/`biome.json` if not using TypeScript
- **Update Claude hooks**: Edit `.claude/settings.json` to remove formatting commands for languages you're not using (e.g., remove `uv run ruff format .` if not using Python, or `bun run format` if not using TypeScript)
...
- `feat`, `fix`, `docs`, `style`, `refactor`, `perf`, `test`, `cicd`, `deps`, `revert`, `WIP`
